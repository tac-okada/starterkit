...
;// CONCATENATED MODULE: ./src/scripts/appIndex.js
function appIndex_typeof(obj) { "@babel/helpers - typeof"; return appIndex_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (obj) { return typeof obj; } : function (obj) { return obj && "function" == typeof Symbol && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }, appIndex_typeof(obj); }
function appIndex_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }
function appIndex_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, appIndex_toPropertyKey(descriptor.key), descriptor); } }
function appIndex_createClass(Constructor, protoProps, staticProps) { if (protoProps) appIndex_defineProperties(Constructor.prototype, protoProps); if (staticProps) appIndex_defineProperties(Constructor, staticProps); Object.defineProperty(Constructor, "prototype", { writable: false }); return Constructor; }
function appIndex_toPropertyKey(arg) { var key = appIndex_toPrimitive(arg, "string"); return appIndex_typeof(key) === "symbol" ? key : String(key); }
function appIndex_toPrimitive(input, hint) { if (appIndex_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (appIndex_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
...
function _possibleConstructorReturn(self, call) { if (call && (appIndex_typeof(call) === "object" || typeof call === "function")) { return call; } else if (call !== void 0) { throw new TypeError("Derived constructors may only return object or undefined"); } return appIndex_assertThisInitialized(self); }
function appIndex_assertThisInitialized(self) { if (self === void 0) { throw new ReferenceError("this hasn't been initialised - super() hasn't been called"); } return self; }
...
var AppIndex = /*#__PURE__*/function (_Core) {
  _inherits(AppIndex, _Core);
  var _super = _createSuper(AppIndex);
  function AppIndex() {
    appIndex_classCallCheck(this, AppIndex);
    return _super.apply(this, arguments);
  }
  appIndex_createClass(AppIndex, [{
    key: "scrollHandler",
    value:
    /*
      使用可能な定数・変数一覧  -----------------------------------------------
    ...
    */
    ...
    value: function loadHandler() {
      ...
      var $loader = $('#loader');
      var $loadingText = $loader.children('div.block-loader').children('p.txt-loader').children('span.txt-number');
      var $loadingBar = $loader.children('div.block-loader').children('p.bar-loader').children('span.bar');
      var percentage = 0;
      var imgCount = 0;
      var imgLoad = imagesloaded_default()('body');
      ...
      imgLoad.on('always', function (instance, image) {
        //console.info(imgCount,imgLoad.images.length,percentage);
        $loadingBar.stop().css('width', '100%').on(_this.transitionEnd, function () {
          $(_this).off(_this.transitionEnd);
          $loadingText.stop().text(100);
          /* 念の為再度this.setupEvents実行（現在のwindowサイズなどを取得のため） */
          _this.setupEvents();
          $('body').toggleClass('loading active');
          $('html').addClass('active');
          $loader.on(_this.animationEnd, function () {
            $(_this).off(_this.animationEnd).remove();
